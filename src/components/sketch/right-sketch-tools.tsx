...
export function RightSketchTools({
  ...
  return (
    <div className="absolute top-1/2 right-4 z-30 flex -translate-y-1/2 items-start gap-3">
      {activePanel === "search" && (
        <Panel title="Search">
        ...
        <Panel title="Layout" pinnable>
          ...
          <Field label="Spacing">
            <div className="grid grid-cols-2 gap-2">
              <NumberInput value={activeFrame?.autoLayout.spacing ?? 0} onChange={(v) => onFrameLayoutChange({ spacing: v })} />
          ...
          <Field label="Layout Guide">
            <div className="flex items-center gap-2 rounded-lg bg-secondary px-2 py-1.5 text-xs">
              ...
              >
                <Eye className={`h-3.5 w-3.5 ${activeFrame?.showGrid ? "text-primary" : "text-muted-foreground"}`} />
              </button>
            </div>
          </Field>
        ...
        <Panel title="Properties" eyeToggle pinnable>
          <Field label="Alignment">
            <div className="grid grid-cols-3 gap-1.5">
              {[AlignLeft, AlignCenter, AlignRight, AlignLeft, AlignCenter, AlignRight, AlignLeft, AlignCenter, AlignRight].map(
                (Icon, i) => (
                  <button
                    key={i}
      ...
      <div className="flex flex-col gap-1 rounded-full border border-border/60 bg-popover p-1.5 shadow-lg">
        {TOOLS.map((t) => (
          <button
            key={t.key}
